...
export class SubTypesController {
    ...
    async getReservationStatus(request: Request, response: Response): Promise<Response> {
        try {
            const reservationStatus: ReservationStatus[] = await getRepository(ReservationStatus).find();
